...
public class HolidayPanel extends JPanel {
    ...
    private void buildUI() {
        ...
        editBtn.addActionListener(e -> {
            ...
            if (idVal == null) {
                UIHelper.showInfo(this, "Select a holiday record to edit.");
                return;
            }
            openForm((int) idVal);
        });
    ...
    private void deleteSelected() {
        Object idVal = tablePanel.getSelectedValue();
        if (idVal == null) {
            UIHelper.showInfo(this, "Select a holiday record first.");
            return;
        }

        int id = (int) idVal;
        if (!UIHelper.confirm(this, "Confirm Delete", "Permanently delete this holiday record?"))
            return;
        try {
    ...
    private void loadDefaults() {
        if (!UIHelper.confirm(this, "Load Defaults", "Load common 2026 holidays into the master list?"))
            return;
        ...
        try {
            ...
            UIHelper.showError(this, "Error loading defaults: " + ex.getMessage());
        }
    }
}
